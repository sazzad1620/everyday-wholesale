...
| Login / Signup UI (mock, no real auth) | ⬜ | "Log In"/"Sign Up" in `account_sheet.dart` currently just flip `AccountBloc` to signed-in local state — no actual form/page. Real pages should still not call Firebase yet; wire to `AccountBloc` the same way, just as dedicated screens instead of inline buttons |
