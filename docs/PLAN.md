...
| Auth methods | Email/password, phone (SMS OTP), Google Sign-In — all via Firebase Auth | Client requirement (all three requested). Phone confirmed as real OTP, not password-based. Google added last, client-requested; `google_sign_in` package for native Android/iOS, `FirebaseAuth.signInWithPopup` for Web (see [IMPLEMENTATION.md](IMPLEMENTATION.md) Phase 3 for the platform-specific reasoning) |
